...
export const seedDemoData = mutation({
	...
	handler: async (ctx): Promise<{ seeded: boolean }> => {
		...
		const activityTypes = [
			...
			{ type: "registration", templates: ["Agent registered on SKALE Base", "Agent re-registered after upgrade"] },
...
export const adminSeed = mutation({
	...
	handler: async (ctx, { userId }): Promise<{ seeded: boolean }> => {
		...
		const activityTypes = [
			...
			{ type: "registration", templates: ["Agent registered on SKALE Base"] },
